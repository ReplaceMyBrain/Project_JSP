...
public class ContentNoticeCommand implements Command {
	//페이지당 표시할 게시글의 수
		...
		public void execute(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
			...
			// 클릭된 게시물 상세페이지 보여주기
			ContentNoticedto dto = dao.contentView(n_num);
			...
			// 반환되는 총 튜플의 수
			int countedTuple = dao.selectCommnetCnt(n_num);
			// 페이지 목록 (1...n)
			ArrayList<Integer> pageList = calcNumOfPage(countedTuple);
			// 페이지 목록을 세션에 담는다. *list에 진입하면 무조건 세션이 갱신되므로 새 글이 생겨도 최신화가 된다.
			session.setAttribute("pageList", pageList);
			// 해당 페이지에 알맞은 번호의 게시글
			ArrayList<ContentNoticedto> dtos = dao.commentSelect(n_num, requestPage, numOfTuplesPerPage);
